...
function updateNavSection() {
    ...
    else if (projectsSection.getBoundingClientRect().y - window.innerHeight / 3 <
        0) {
        currentSectionBtns = projectsSecBtns;
        projectsSecBtns.forEach(updateBtns);
    }
    else if (skillsSection.getBoundingClientRect().y - window.innerHeight / 3 <
        0) {
        currentSectionBtns = skillsSecBtns;
        skillsSecBtns.forEach(updateBtns);
    }
    else if (aboutMeSection.getBoundingClientRect().y - window.innerHeight / 3 <
        0) {
...
function initHeader() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let visits = "";
        try {
            const res = yield fetch("https://api.crc.cloudydaiyz.com/portfolio", {
                method: "POST",
            });
